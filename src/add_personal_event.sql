 ...
 ELSE
  ...
  IF !immutable THEN
   ...
   INSERT INTO schedule_item(date, start_time, duration, location,
    event_name, schedule_id, immutable, deadline_date, deadline_time)
	VALUES(eventDate, SEC_TO_TIME(TIME_TO_SEC(startTime) - TIME_TO_SEC(eventDuration)),
	eventDuration, eventLocation, eventName, sid, immutable, eventDate, startTime);
  ELSE
   INSERT INTO schedule_item(date, start_time, duration, location,
    event_name, schedule_id, immutable) VALUES(eventDate, startTime,
	eventDuration, eventLocation, eventName, sid, immutable);
  END IF;
  SET success = 1;
 END IF;
END$$
